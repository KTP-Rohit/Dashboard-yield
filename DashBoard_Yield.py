...
while True:
    ...
    giveaway_summary = Giveaway(date.strftime('%Y-%m-%d'),connection_string)
    df3 = giveaway_summary.get_data()
    ...
    if df3 is not None:
        with giveaway_element:
            st.subheader("Giveaway Yield")
            st.dataframe(df3)
